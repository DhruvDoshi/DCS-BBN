...
def hash_block(block):
    #hashes a block and returnes the strong representation of it. 
    #return '-'.join([str(block[key]) for key in block])
    #ideally to make system more safer use sha 512
    #return hashlib.sha512(json.dumps(block).encode()).hexdigest()
    '''
    now we had blocks and transactions as dictonary hence as per the charasteristics they are unordered pairs 
    so the same string can he of unorderd but as per our hash logic if there is a single change in the string
    the newly generated hash would be diferent now this is out technical fault
    due to this even if our block is not manupulated the newly generates hash would give us error
    to solve this we have way in jason files to sort then so they will remain in same order till they are 
    manupulated from the outside and this will figure out problem out
    '''
